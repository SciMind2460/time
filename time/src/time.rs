...
impl ToSql for Time {
    fn to_sql(&self) -> Result<ToSqlOutput<'_>> {
        let (hour, minute, second, millisecond) = self.as_hms_milli();
        let formatted_time = format!("{hour}:{minute}:{second}.{millisecond}");
        Ok(ToSqlOutput::Owned(Value::Text(formatted_time)))
    }
}
